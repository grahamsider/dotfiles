...
CURR_CONF_DIR="$XDPI_CONF_DIR/lt-1080p"
i3_DIFF_PATH="$XDPI_DIFF_DIR/i3.diff"

# Get pulseaudio card index
AUDIO_CARD_INDEX=$(pacmd list-cards | grep "index" | sed "s/^.*index:\s*//")

# Keep on laptop display, turn on 1080p display
xrandr --output eDP1 --primary --mode 3000x2000 --pos 0x0 --rotate normal --output DP1 --mode 1920x1080 --pos 3000x407 --rotate normal --output DP2 --off --output HDMI1 --off --output VIRTUAL1 --off
